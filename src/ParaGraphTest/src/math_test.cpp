...
void tensor_construction_test::run() const {
	...
	assert(t[19] == 19, "tensor::tensor should copy data correctly.");
...
void tensor_zero_test::run() const {
	...
	std::for_each(zero.begin(), zero.end(),
...
void tensor_scalar_test::run() const {
	...
	assert_doubles_are_close(v, scalar[0], 1e-15,
...
void tensor_identity_derivative_test::run() const {
	...
	data_size *= data_size;
	assert(I.size() == data_size, "tensor::identity_derivative should return correct data size.");
	...
	for (std::size_t i_data = 0; i_data < data.size(); ++i_data) {
		assert_doubles_are_close(data[i_data], I[i_data], 1e-15,
			...
			for (std::size_t i5 = 0; i5 < bn4; ++i5) {
				...
				for (std::size_t i2 = 0; i2 < an2; ++i2)
					for (std::size_t i3 = 0; i3 < an3; ++i3) {
						double ad = a[a.compute_offset(tensor::N_vector { i1, i2, i3 })];
						double bd = b[b.compute_offset(tensor::N_vector { i2, i3, i4, i5 })];
						cd_expected += ad * bd;
					}
				double cd = c[c.compute_offset(tensor::N_vector { i1, i4, i5 })];
...
void tensor_add_test::run() const {
	...
	assert(ta.size() == n1 * n2, "tensor::add must return data of correct size.");
	for (std::size_t i = 0; i < n1 * n2; ++i) {
		assert_doubles_are_close(ta[i], t1[i] + t2[i], 1e-15, "tensor::add must return correct data.");
	}

}

std::string tensor_iterator_test::name() const {
	return "tensor_iterator_test";
}

template<typename TIterator>
void testIterators(TIterator const & begin, TIterator const & end, tensor const & t)
{
	TIterator start_plus_i = begin;
	TIterator end_minus_i = end;
	std::size_t ts = t.size();
	for (std::size_t i = 0; i < ts; ++i)
	{
		assert_doubles_are_close(*start_plus_i, t[i], 1e-15,
				"iterator moved forward must have correct value.");
		assert_doubles_are_close(*end_minus_i, t[ts - i], 1e-15,
				"iterator moved backward must have correct value.");
		assert(start_plus_i - begin == i,
				"difference for fwd moved iterator should be ",
				i,
				".");
		assert(end_minus_i - begin == (ts - i),
				"difference for backward iterator should be " ,
				(ts - i),
				".");
		assert(start_plus_i == begin + i,
				"iterator + ",
				i, " should work correctly.");
		assert(end_minus_i == end - i,
				"iterator - ",
				i, " should work correctly.");
		assert((start_plus_i < end_minus_i) == (i < (ts - i)),
				"tensor::iterator::operator < should be correct");
		assert((start_plus_i > end_minus_i) == (i > (ts - i)),
						"tensor::iterator::operator > should be correct");
		assert((start_plus_i <= end_minus_i) == (i <= (ts - i)),
						"tensor::iterator::operator <= should be correct");
...
void tensor_iterator_test::run() const {
	tensor::N const n1 = 2, n2 = 3;
	tensor::N_vector dims { n1, n2 };

	std::default_random_engine dre;
	std::uniform_real_distribution<double> urd(0, 1);

	std::vector<double> data(n1 * n2);
	std::for_each(data.begin(), data.end(), [&](double & d) { d = urd(dre);});

	tensor t(dims, data);

	assert(t.size() == data.size(), "tensor::size must return correct size");


	assert_doubles_are_close(*t.begin(), data[0], 1e-15, "tensor::begin must contain first element.");
	assert_doubles_are_close(*(static_cast<tensor const>(t)).begin(), data[0], 1e-15, "tensor::begin const must contain first element.");
	assert_doubles_are_close(*t.cbegin(), data[0], 1e-15, "tensor::cbegin must contain first element.");

	testIterators(t.begin(), t.end(), t);
	const tensor & tconst = t;
	testIterators(tconst.begin(), tconst.end(), t);
	testIterators(t.cbegin(), t.cend(), t);
}
